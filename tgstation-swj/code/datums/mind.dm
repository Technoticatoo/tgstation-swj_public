...
/datum/mind
	...
	var/list/spell_list = list() // Jedi mode & "Give Spell" badmin button.
...
/datum/mind/proc/equip_traitor(employer = "The Rebel", silent = FALSE, datum/antagonist/uplink_owner)
	if(!current)
		return
	...
	if (!uplink_loc)
		...
		if(!silent)
			if(uplink_loc == R)
				...
				to_chat(traitor_mob, "[employer] has cunningly disguised a Rebel Uplink as your [P.name]. Simply twist the top of the pen [U.unlock_code] from its starting position to unlock its hidden features.")
...
/datum/mind/proc/enslave_mind_to_creator(mob/living/creator)
	...
	else if(is_rebel(creator))
		var/datum/antagonist/rev/converter = creator.mind.has_antag_datum(/datum/antagonist/rev,TRUE)
		converter.add_rebel(src,FALSE)
		...
		switch(href_list["common"])
			...
			if("crystals")
				if(check_rights(R_FUN, 0))
					...
					if(U)
						var/crystals = input("Amount of telecrystals for [key]","Rebel uplink", U.telecrystals) as null | num
						if(!isnull(crystals))
