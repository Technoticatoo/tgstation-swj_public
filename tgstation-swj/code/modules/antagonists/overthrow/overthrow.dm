...
// Rebel mutineer agents. They're agents selected by the Rebel to take control of stations when assault teams like nuclear operatives cannot be sent.
...
// to a storage implant they came with when the Rebel sent them aboard, with one random low-cost traitor item. The initial agent also has this. The only difference between
...
/datum/antagonist/overthrow
	...
	antagpanel_category = "Rebel Mutineers"
...
/datum/antagonist/overthrow/proc/equip_initial_overthrow_agent()
	...
	if (!where)
		to_chat(H, "The Rebel were unfortunately unable to get you the AI module.")
	else
		to_chat(H, "Use the AI board in your [where] to take control of the AI, as requested by the Rebel.")
	// Give the implant converter
	var/obj/item/overthrow_converter/I = new(H)
	where = H.equip_in_one_of_slots(I, slots)
	if (!where)
		to_chat(H, "The Rebel were unfortunately unable to get you a converter implant.")
	else
...
/datum/antagonist/overthrow/greet()
	to_chat(owner.current, "<B><font size=3 color=red>You are a syndicate sleeping agent!</font> <font size=2 color=red>Your job is to stage a swift, fairly bloodless coup. Your team has a two-use converter that can be used to convert \
			anyone you want, although mind shield implants need to be removed firstly for it to work. Your team also has a special version of the Rebel module to be used to convert the AI, too. You \
			...
			take control of the station faster, so it should be prioritized, especially over killing, which should be avoided where possible. The other Rebel teams are NOT friends and should not \
			be trusted.</font></B>")
