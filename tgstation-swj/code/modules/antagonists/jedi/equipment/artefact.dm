...
/obj/item/veilrender
	...
	var/charges = 1
	var/spawn_type = /obj/singularity/jedi
	var/spawn_amt = 1
...
/obj/singularity/jedi/process()
	move()
	eat()
	return

/obj/singularity/jedi/attack_tk(mob/user)
	if(iscarbon(user))
		...
		for(var/lore in typesof(/datum/brain_trauma/severe))
			C.gain_trauma(lore)
		addtimer(CALLBACK(src, /obj/singularity/jedi.proc/deranged, C), 100)

/obj/singularity/jedi/proc/deranged(mob/living/carbon/C)
	if(!C || C.stat == DEAD)
		return
...
/obj/singularity/jedi/mapped/admin_investigate_setup()
	return
...
/obj/item/organ/heart/cursed/jedi
	pump_delay = 60
	heal_brute = 25
	heal_burn = 25
	heal_oxy = 25
