/datum/spellbook_entry
	...
	var/buy_word = "Learn"
	var/limit //used to prevent a spellbook_entry from being bought more than X times with one jedi spellbook
	...
	for(var/obj/effect/proc_holder/spell/aspell in user.mind.spell_list)
		if(initial(S.name) == initial(aspell.name)) // Not using directly in case it was learned from one spellbook then upgraded in another
			if(aspell.spell_level >= aspell.level_max)
				...
				SSblackbox.record_feedback("nested tally", "jedi_spell_improved", 1, list("[name]", "[aspell.spell_level]"))
				return TRUE
	//No same spell found - just learn it
	SSblackbox.record_feedback("tally", "jedi_spell_learned", 1, name)
...
/datum/spellbook_entry/proc/Refund(mob/living/carbon/human/user,obj/item/spellbook/book) //return point value or -1 for failure
	var/area/jedi_station/A = GLOB.areas_by_type[/area/jedi_station]
	if(!(user in A.contents))
		to_chat(user, "<span class='warning'>You can only refund spells at the jedi lair</span>")
		return -1
	if(!S)
		S = new spell_type()
	var/spell_levels = 0
...
/datum/spellbook_entry/proc/GetInfo()
	...
	dat += "[S.clothes_req?"Requires jedi garb.":"Can be cast without jedi garb."]<br>"
	return dat
...
/datum/spellbook_entry/item/armor
	...
	item_path = /obj/item/clothing/suit/space/hardsuit/jedi
	category = "Defensive"
...
/datum/spellbook_entry/item/contract
	...
	desc = "A magical contract binding an apprentice jedi to your service, using it will summon them to your side."
...
/datum/spellbook_entry/item/guardian
	...
	item_path = /obj/item/guardiancreator/choose/jedi
	category = "Assistance"

/datum/spellbook_entry/item/guardian/Buy(mob/living/carbon/human/user,obj/item/spellbook/book)
	. = ..()
	if(.)
		new /obj/item/paper/guides/antag/guardian/jedi(get_turf(user))
...
/datum/spellbook_entry/item/battlemage
	...
	item_path = /obj/item/clothing/suit/space/hardsuit/shielded/jedi
	limit = 1
	category = "Defensive"
...
/datum/spellbook_entry/item/battlemage_charge
	...
	item_path = /obj/item/jedi_armour_charge
	category = "Defensive"
	cost = 1
...
/datum/spellbook_entry/summon/ghosts/Buy(mob/living/carbon/human/user, obj/item/spellbook/book)
	SSblackbox.record_feedback("tally", "jedi_spell_learned", 1, name)
	new /datum/round_event/jedi/ghost()
	active = TRUE
...
/datum/spellbook_entry/summon/events
	name = "Summon Events"
	desc = "Give Murphy's law a little push and replace all events with special jedi ones that will confound and confuse everyone. Multiple castings increase the rate of these events."
	var/times = 0
...
/obj/item/spellbook/proc/GetCategoryHeader(category)
	...
	switch(category)
		...
		if("Assistance")
			...
			dat += "You can reduce this number by spending more points on the spell.<BR>"
		if("Challenges")
			dat += "The Jedi Federation typically has hard limits on the potency and number of spells brought to the station based on risk.<BR>"
...
/obj/item/spellbook/Topic(href, href_list)
	...
	if(H.mind.special_role == "apprentice")
		temp = "If you got caught sneaking a peek from your teacher's spellbook, you'd likely be expelled from the Jedi Academy. Better not."
		return
