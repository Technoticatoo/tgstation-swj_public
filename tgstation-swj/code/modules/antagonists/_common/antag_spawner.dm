	...
	if(loc == H || (in_range(src, H) && isturf(loc)))
		...
		if(href_list["school"])
			if(used)
				...
				return
			var/list/candidates = pollCandidatesForMob("Do you want to play as a jedi's [href_list["school"]] apprentice?", ROLE_WIZARD, null, ROLE_WIZARD, 150, src)
...
/obj/item/antag_spawner/contract/spawn_antag(client/C, turf/T, kind ,datum/mind/user)
	...
	var/datum/antagonist/jedi/apprentice/app = new()
	app.master = user
	app.school = kind

	var/datum/antagonist/jedi/master_jedi = user.has_antag_datum(/datum/antagonist/jedi)
	if(master_jedi)
		if(!master_jedi.wiz_team)
			master_jedi.create_wiz_team()
		app.wiz_team = master_jedi.wiz_team
		master_jedi.wiz_team.add_member(app_mind)
...
/obj/item/antag_spawner/nuke_ops/attack_self(mob/user)
	...
	if(LAZYLEN(nuke_candidates))
		...
		to_chat(user, "<span class='warning'>Unable to connect to Rebel command. Please wait and try again later or use the teleporter on your uplink to get your points refunded.</span>")
...
/obj/item/antag_spawner/nuke_ops/borg_tele/spawn_antag(client/C, turf/T, kind, datum/mind/user)
	...
	R.mind.special_role = "Rebel Cyborg"
