...
/mob/living/simple_animal/bot/med_droid
	name = "\improper Medical droid"
	desc = "A medical robot. He will cure your ills! What? No! I don't mean cause!."
...
/mob/living/simple_animal/bot/med_droid/mysterious
	name = "\improper Mysterious Medical droid"
	desc = "Imperial Medical droid of mystery."
	skin = "bezerk"
	treatment_brute = "tricordrazine"
	treatment_fire = "tricordrazine"
	treatment_tox = "tricordrazine"

/mob/living/simple_animal/bot/med_droid/derelict
	name = "\improper Old Medical droid"
...
/mob/living/simple_animal/bot/med_droid/update_icon()
	cut_overlays()
	if(skin)
...
/mob/living/simple_animal/bot/med_droid/Initialize(mapload, new_skin)
	. = ..()
...
/mob/living/simple_animal/bot/med_droid/bot_reset()
	..()
	patient = null
	oldpatient = null
	oldloc = null
...
/mob/living/simple_animal/bot/med_droid/proc/soft_reset() //Allows the medibot to still actively perform its medical duties without being completely halted as a hard reset does.
	path = list()
	patient = null
	mode = BOT_IDLE
	last_found = world.time
...
/mob/living/simple_animal/bot/med_droid/set_custom_texts()
...
/mob/living/simple_animal/bot/med_droid/attack_paw(mob/user)
	return attack_hand(user)

/mob/living/simple_animal/bot/med_droid/get_controls(mob/user)
	var/dat
	dat += hack(user)
	dat += showpai(user)
...
/mob/living/simple_animal/bot/med_droid/Topic(href, href_list)
	if(..())
		return 1
...
/mob/living/simple_animal/bot/med_droid/attackby(obj/item/W as obj, mob/user as mob, params)
...
/mob/living/simple_animal/bot/med_droid/emag_act(mob/user)
	..()
	if(emagged == 2)
		declare_crit = 0
		if(user)
...
/mob/living/simple_animal/bot/med_droid/process_scan(mob/living/carbon/human/H)
	if(H.stat == DEAD)
		return
...
/mob/living/simple_animal/bot/med_droid/handle_automated_action()
	if(!..())
		return
...
/mob/living/simple_animal/bot/med_droid/proc/assess_patient(mob/living/carbon/C)
...
/mob/living/simple_animal/bot/med_droid/UnarmedAttack(atom/A)
	if(iscarbon(A))
...
/mob/living/simple_animal/bot/med_droid/examinate(atom/A as mob|obj|turf in view())
	..()
	if(!is_blind(src))
		chemscan(src, A)

/mob/living/simple_animal/bot/med_droid/proc/medicate_patient(mob/living/carbon/C)
	if(!on)
		return
...
/mob/living/simple_animal/bot/med_droid/proc/check_overdose(mob/living/carbon/patient,reagent_id,injection_amount)
...
/mob/living/simple_animal/bot/med_droid/explode()
	on = FALSE
...
/mob/living/simple_animal/bot/med_droid/proc/declare(crit_patient)
...
/obj/machinery/bot_core/med_droid
