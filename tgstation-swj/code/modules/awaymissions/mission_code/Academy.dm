...
/obj/structure/academy_jedi_spawner
	...
	anchored = TRUE
	max_integrity = 200
	var/mob/living/current_jedi = null
	var/next_check = 0
	var/cooldown = 600
	var/faction = ROLE_WIZARD
	var/braindead_check = 0

/obj/structure/academy_jedi_spawner/New()
	START_PROCESSING(SSobj, src)

/obj/structure/academy_jedi_spawner/Destroy()
	if(!broken)
		STOP_PROCESSING(SSobj, src)
	return ..()

/obj/structure/academy_jedi_spawner/process()
	if(next_check < world.time)
		if(!current_jedi)
			...
			if(current_jedi.stat == DEAD)
				current_jedi = null
				summon_jedi()
			if(!current_jedi.client)
				if(!braindead_check)
					braindead_check = 1
				else
					braindead_check = 0
...
/obj/structure/academy_jedi_spawner/proc/give_control()
	set waitfor = FALSE

	if(!current_jedi)
		return
	var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as Jedi Academy Defender?", ROLE_WIZARD, null, ROLE_WIZARD, 50, current_jedi)

	if(LAZYLEN(candidates))
		var/mob/dead/observer/C = pick(candidates)
		message_admins("[ADMIN_LOOKUPFLW(C)] was spawned as Jedi Academy Defender")
		current_jedi.ghostize() // on the off chance braindead defender gets back in
		current_jedi.key = C.key

/obj/structure/academy_jedi_spawner/proc/summon_jedi()
	var/turf/T = src.loc
	...
	wizmind.add_antag_datum(/datum/antagonist/jedi/academy)
	current_jedi = wizbody

	give_control()

/obj/structure/academy_jedi_spawner/deconstruct(disassembled = TRUE)
	if(!broken)
		broken = 1
...
/obj/item/dice/d20/fate/diceroll(mob/user)
	..()
	if(!used)
		if(!ishuman(user) || !user.mind || (user.mind in SSticker.mode.jedis))
...
/obj/structure/ladder/unbreakable/rune/use(mob/user, is_ghost=FALSE)
	if(is_ghost || !(user.mind in SSticker.mode.jedis))
		..()
