/obj/pod/process(time)
	...
	if(angular_velocity_adjustment && power_source && power_source.use(abs(angular_velocity_adjustment) * 0.05))
	...
	if(velocity_mag || angular_velocity != 0)
		...
		for(var/turf/T in locs)
			...
			drag += 0.001
			var/floating = FALSE
			if(T.has_gravity() && !brakes && velocity_mag > 0.1 && power_source && power_source.use((is_mining_level(z) ? 3 : 15) * time))
	...
	if(power_source && power_source.use(10 * sqrt((thrust_x*thrust_x)+(thrust_y*thrust_y)) * time))
	...
	while((offset_x > 0 && velocity_x > 0) || (offset_y > 0 && velocity_y > 0) || (offset_x < 0 && velocity_x < 0) || (offset_y < 0 && velocity_y < 0))
		...
		if(failed_x && failed_y)
			break
	// prevents situations where you go "wtf I'm clearly right next to it" as you enter a stationary pod
	if(velocity_x == 0)
		if(offset_x > 0.5)
			if(Move(get_step(src, EAST)))
				offset_x--
...
/obj/pod/Bump(atom/A)
	...
	if(bump_velocity > 10 && !ismob(A))
		...
		message_admins("[key_name_admin(pilot)] has impacted a pod into a wall with velocity [bump_velocity]")
		take_damage(strength*10, BRUTE, "melee", TRUE)
		log_game("[key_name(pilot)] has impacted a pod into a wall with velocity [bump_velocity]")
...
/obj/pod/proc/fire_projectiles(proj_type, target) // if pods of other sizes are added override this or something
	var/fx = cos(90 - angle)
	var/fy = sin(90 - angle)
	var/sx = fy
	var/sy = -fx
