...
/datum/admins/Topic(href, href_list)
	...
	if(href_list["ahelp"])
		...
		switch(href_list["makeAntag"])
			...
			if("revs")
				if(src.makeRevs())
					message_admins("[key_name(usr)] started a rebellion.")
					log_admin("[key_name(usr)] started a rebellion.")
				else
					message_admins("[key_name_admin(usr)] tried to start a rebellion. Unfortunately, there were no candidates available.")
					log_admin("[key_name(usr)] failed to start a rebellion.")
			if("cult")
				if(src.makeCult())
...
//Regular jobs
	...
	//Antagonist (Orange)
		...
		if(jobban_isbanned(M, ROLE_WIZARD) || isbanned_dept)
			dat += "<td width='20%'><a href='?src=[REF(src)];[HrefToken()];jobban3=jedi;jobban4=[REF(M)]'><font color=red>Jedi</font></a></td>"
		else
			dat += "<td width='20%'><a href='?src=[REF(src)];[HrefToken()];jobban3=jedi;jobban4=[REF(M)]'>Jedi</a></td>"
