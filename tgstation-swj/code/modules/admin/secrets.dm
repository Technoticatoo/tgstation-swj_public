/datum/admins/proc/Secrets()
	...
	if(check_rights(R_DEBUG,0))
		dat += {"
			...
			<A href='?src=[REF(src)];[HrefToken()];secrets=infinite_sec'>Remove cap on Stormtroopers</A><BR>
			<BR>
			"}
...
/datum/admins/proc/Secrets_topic(item,href_list)
	...
	switch(item)
		...
		if("infinite_sec")
			...
			J.total_positions = -1
			J.spawn_positions = -1
			message_admins("[key_name_admin(usr)] has removed the cap on Stormtroopers.")
