...
/obj/item/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(M.mind == target && !M.owns_soul())
		if(user.mind && (user.mind.assigned_role == "Lawyer"))
			deconvert = TRUE
		else if (user.mind && (user.mind.assigned_role =="Lieutenant") || (user.mind.assigned_role == "CentCom Commander"))
			deconvert = prob (25) // the HoP doesn't have AS much legal training
		else
			deconvert = prob (5)
	if(deconvert)
		M.visible_message("<span class='notice'>[user] reminds [M] that [M]'s soul was already purchased by Imperial!</span>")
		to_chat(M, "<span class='boldnotice'>You feel that your soul has returned to its rightful owner, Imperial.</span>")
		M.return_soul()
	else
