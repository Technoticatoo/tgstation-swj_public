...
/obj/machinery/computer/secure_data/laptop
	...
	desc = "A cheap Imperial security laptop, it functions as a security records console. It's bolted to the table."
	icon_state = "laptop"
...
<th>Time Added</th>
			...
			if("Print Poster")
				if(!( printing ))
					...
					if(wanted_name)
						var/default_description = "A poster declaring [wanted_name] to be a dangerous individual, wanted by the Empire. Report any sightings to security immediately."
				...
				switch(href_list["field"])
					...
					if("criminal")
						if(istype(active2, /datum/data/record))
							...
							temp += "</ul>"
					if("rank")
						var/list/L = list( "Lieutenant", "Captain", "AI", "Central Command" )
