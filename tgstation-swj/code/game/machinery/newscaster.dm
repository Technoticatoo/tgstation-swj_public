	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			if(0)
				...
				dat += "<BR><FONT SIZE=1>Property of the Empire</FONT>"
				if(GLOB.news_network.wanted_issue.active)
			...
			if(9)
				...
				if(viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with an Imperial D-Notice.<BR>"
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
				...
				if(choice=="Confirm")
					...
					if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one.
						...
						if(GLOB.news_network.wanted_issue.isAdminMsg)
							alert("The wanted issue has been distributed by an Imperial higherup. You cannot edit it.","Ok")
							return
			...
			if(GLOB.news_network.wanted_issue.isAdminMsg)
				alert("The wanted issue has been distributed by an Imperial higherup. You cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by an Imperial Officer. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by an Imperial Officer. You cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by an Imperial Officer. You cannot place a D-Notice upon it.","Ok")
				return
...
/obj/item/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard an Imperial Space Stations."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>Imperial-standard newspaper, for use on Imperial? Space Facilities</FONT></div><HR>"
