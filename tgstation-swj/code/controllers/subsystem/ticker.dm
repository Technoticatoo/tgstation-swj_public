...
/datum/controller/subsystem/ticker/proc/send_news_report()
	var/news_message
	var/news_source = "Imperial News Network"
	switch(news_report)
		...
		if(STATION_DESTROYED_NUKE)
			news_message = "We would like to reassure all employees that the reports of a Rebel backed nuclear attack on [station_name()] are, in fact, a hoax. Have a secure day!"
		...
		if(NUKE_MISS)
			news_message = "The Rebel have bungled a terrorist attack [station_name()], detonating a nuclear weapon in empty space nearby."
		if(OPERATIVES_KILLED)
			news_message = "Repairs to [station_name()] are underway after an elite Rebel death squad was wiped out by the crew."
		if(OPERATIVE_SKIRMISH)
			news_message = "A skirmish between security forces and Rebel agents aboard [station_name()] ended with both sides bloodied but intact."
		if(REVS_WIN)
